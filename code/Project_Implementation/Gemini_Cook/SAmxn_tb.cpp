#include "SAmxn.h" // This now includes both SA_MxN and MatMul_Controller
#include <iostream>
...
// --- Test Configuration ---
// =================================================================
// M and N are the PHYSICAL grid dimensions (from SAmxn.h)
// K1, K2, K3 are the LOGICAL matrix dimensions
// C[K1][K3] = A[K1][K2] * W[K2][K3]

// Set logical dimensions different from physical M,N to test tiling
#ifndef K1_DIM
#define K1_DIM 50  // e.g., Output Rows
#endif

#ifndef K2_DIM
#define K2_DIM 6  // e.g., Inner Dimension (maps to M)
#endif

#ifndef K3_DIM
#define K3_DIM 1  // e.g., Output Columns (maps to N)
#endif
...
SC_MODULE(SA_MxN_tb)
{
    ...
    SC_CTOR(SA_MxN_tb) : clk("clk", 10, SC_NS)
    {
        ...
        // Fill A and W with simple, predictable values
        for (int i = 0; i < K1_DIM; ++i)
            for (int j = 0; j < K2_DIM; ++j)
                A_mat[i * K2_DIM + j] = i + 1; // Row i has value i+1

        for (int i = 0; i < K2_DIM; ++i)
            for (int j = 0; j < K3_DIM; ++j)
                W_mat[i * K3_DIM + j] = j + 1; // Col j has value j+1
...
//         // This accesses signals *inside* the controller instance
//         cout << "Tracing internal Systolic Array I/O..." << endl;
        
//         sc_trace(tf, system.controller->sa_reset, "controller.sa_reset");
//         sc_trace(tf, system.controller->sa_output_stationary, "controller.sa_output_stationary");
...
//         // Uncomment to trace preload data (can be very large!)
//         // for (int i = 0; i < M * N; i++) {
//         //     string name = "controller.sa_preload_data_" + std::to_string(i);
//         //     sc_trace(tf, system.controller->sa_preload_data[i], name);
//         // }
...
int sc_main(int argc, char* argv[]) {
    ...
    if (tf) {
        ...
        // --- Trace Internal SA Grid I/O (Hierarchical Tracing) ---
        // This accesses signals *between* the controller and the grid
        cout << "Tracing internal Systolic Array I/O..." << endl;
        ...
        // --- NEW: Trace all PE-level signals ---
        cout << "Tracing ALL PE internal signals..." << endl;
        for (int i = 0; i < M; ++i) {
            for (int j = 0; j < N; ++j) {
                // Create a clear hierarchical name for each PE
                string pe_name = "controller.sa_grid.PE_" 
                                 + to_string(i) + "_" + to_string(j);
                
                // Get the pointer to the PE instance
                ...
                // --- Trace PE Internal Buffers/State ---
                // Note: These must be member variables of the PE class, not local to a process
