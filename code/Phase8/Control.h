...
SC_MODULE(MemoryBackedController)
{
    ...
    float* W_tile;
    float* C_tile;
    int tile_stride;  // Buffer stride = max(M, N)
    
    SC_CTOR(MemoryBackedController)
    {
        tile_stride = (M > N) ? M : N;  // max(M, N)
    ...
    void control_process()
    {
        ...
        while (true)
        {
            ...
            // - Col tiles: N cols (use full PE column width)
            ...
            for (int i_tile = 0; i_tile < num_i_tiles; i_tile++) {
                for (int j_tile = 0; j_tile < num_j_tiles; j_tile++) {
                    ...
                    for (int i = 0; i < tile_stride * tile_stride; i++) C_tile[i] = 0.0f;
