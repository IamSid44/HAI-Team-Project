...
SC_MODULE(MatMul_Controller)
{
   ...
   // FIX: Corrected all stride usage to use buffer stride M instead of matrix strides
   // =================================================================
    void tiling_process() 
    {
        // Initial reset
        ...
        while(true) 
        {
            ...
            if (sa_mode_is_output_stationary.read() == false)
            {
                ...
                for (int i = 0; i < M; ++i) { 
                    for (int j = 0; j < N; ++j) { 
                        // CRITICAL FIX: Use buffer stride M, not matrix stride k3
                        // Buffers are always stored with stride M
                        if (i < k2 && j < k3) {
                ...
                // Total cycles needed: k1 (input depth) + M (skew) + N (drain)
                int total_cycles = k1 + M + N - 1;
                
                for (int clk_cycle = 0; clk_cycle < total_cycles; ++clk_cycle) 
                {
                    // Feed A_tile with skew
                    for (int i = 0; i < M; ++i) { 
                        int a_row = clk_cycle - i; // Skewed timing
                        
                        if (a_row >= 0 && a_row < k1 && i < k2) {
                            // CRITICAL FIX: Use buffer stride M, not k2
                    ...
                    for (int j = 0; j < N; ++j) { 
                        // WS timing: row r's result exits at cycle r + M + j
                        // Therefore: r = clk - M - j
                        int r_out = clk_cycle - M - j; 
                        
                        if (r_out >= 0 && r_out < k1 && j < k3) {
                            float partial_sum = sa_out_bottom[j].read();
                            // CRITICAL FIX: Use buffer stride M, not k3
            ...
            else 
            {
                ...
                for (int clk_cycle = 0; clk_cycle < stream_cycles; ++clk_cycle)
                {
                    ...
                    for (int j = 0; j < N; ++j) {
                        int k = clk_cycle - j; // K-index considering skew
                ...
                // FIX: Only need 1 wait() to trigger drain, not 2
                wait();
                ...
                {
                    for (int j = 0; j < N; ++j) {
                        // OS timing: row (M-1-clk) exits at column j during cycle clk
                        int i_out = M - 1 - clk_cycle;
                        
                        if (i_out >= 0 && i_out < k1 && j < k3) {
                            // CRITICAL FIX: Use buffer stride M, not k3
                            C_ptr[i_out * M + j] = sa_out_bottom[j].read();
                        }
                    }
                    wait();
                }
